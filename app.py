...
st.title(📊 Dashboard - MBF Produção)
...
if arquivo
    ...
    df_op[Data Término] = pd.to_datetime(df_op[Data Término])

    df_op[Mes] = df_op[Data Cadastro].dt.strftime(%b%y)

    # ==============================
    # KPI 1 - OPs no mês atual
    # ==============================
    ...
    aba1, aba2 = st.tabs([📦 OP, 🔁 OPR])

    with aba1

        col1, col2, col3 = st.columns(3)

        col1.metric(OP's no mês, op_mes)
        col2.metric(Tempo médio liberação (d.u.), round(tempo_medio,1))
        col3.metric(Demanda Atual, Em construção)

        st.subheader(OPs por mês)
